...
| **Open work / the board**              | [`tasks/`](tasks/INDEX.md) — one file per task (status+owner, agent queue); grouped INDEX                                                            |
| **Someday / maybe capture**            | [`BACKLOG.md`](BACKLOG.md) — cheap lines; promote to a task when real                                                                                |
...
- Doc types are defined in [`.lore/types/`](.lore/types/) (lore schemas) — decisions, devlog, providers, plans, specs, marketing are one-file-per-record with generated INDEXes (`lore reindex <type>`); BACKLOG.md (capture lines) and architecture.md stay consolidated.
