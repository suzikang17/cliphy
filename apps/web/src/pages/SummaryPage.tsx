...
import type { DesignSection } from "../components/DesignPlayground";
import * as api from "../lib/api";

// Design mode is a separate layout component — code-split, loaded only when used.
...
export function SummaryPage() {
  ...
  const [layout, setLayout] = useState<"plain" | "design">(() => {
    const saved =
      typeof localStorage !== "undefined" ? localStorage.getItem("summaryLayout") : null;
    return saved === "design" ? "design" : "plain";
  });
  useEffect(() => {
    try {
      localStorage.setItem("summaryLayout", layout);
    } catch {
      /* ignore */
    }
  }, [layout]);
  ...
  // ── Reusable section fragments — arranged differently per layout ──────────

  const playerEl = playerError ? (
    <a
  ...
  const metaEl = (
    <div className="min-w-0">
      {/* Title, then channel · duration on the next line */}
      <a
        href={`https://youtube.com/watch?v=${summary.videoId}`}
        target="_blank"
        rel="noopener noreferrer"
      ...
      >
        <h1 className="text-lg font-bold leading-snug m-0 hover:text-neon-600 transition-colors">
          {summary.videoTitle || summary.videoId}
        </h1>
      </a>
      <div className="flex items-center gap-2 text-sm text-(--color-text-muted) mt-1">
  ...
  const truncatedEl = json?.truncated ? (
    <div className="bg-amber-50 dark:bg-amber-950/30 border border-amber-300 dark:border-amber-700 rounded-xl px-4 py-3 text-sm text-amber-700 dark:text-amber-400">
      This summary is based on a partial transcript. The video was too long to process in full.
    </div>
  ) : null;
  ...
  const highlightsEl =
    json && json.keyPoints.length > 0 ? (
      <section className="bg-(--color-surface-raised) rounded-xl p-5">
        <h2 className="text-xs font-bold uppercase tracking-wide text-neon-600 mb-2">Highlights</h2>
  ...
  const jumpToEl =
    json && json.timestamps.length > 0 ? (
      <section className="bg-(--color-surface-raised) rounded-xl px-3 py-4">
        <h2 className="text-xs font-bold uppercase tracking-wide text-neon-600 mb-2 px-1">
          Jump To
        </h2>
        {/* auto-width timestamp column: right-aligned (keeps the colons lined up)
            but only as wide as the longest timestamp — no wasted indent */}
        <div className="grid grid-cols-[auto_1fr] gap-y-0.5 text-sm">
          {json.timestamps.map((ts, i) => {
            const parsed = extractTimestamp(ts);
            if (!parsed) {
              return (
                <div key={i} className="col-span-2 text-(--color-text-body) px-1 py-1">
                  {ts}
                </div>
              );
            }
            const isActive = parsed.seconds === activeChapterSeconds;
            const rowBg = isActive ? "bg-neon-100 dark:bg-neon-900/30" : "";
            const timeCls = `text-right font-mono text-xs font-bold py-1 pl-1 pr-2 rounded-l-md ${rowBg}`;
            const labelCls = `text-left py-1 pr-1 rounded-r-md ${rowBg}`;
              ...
              return (
                <Fragment key={i}>
                  <a
                    ...
                    target="_blank"
                    rel="noopener noreferrer"
                    className={`${timeCls} no-underline text-neon-600 hover:text-neon-800 transition-colors`}
                  >
                    {parsed.time}
                  </a>
                  <span className={`${labelCls} text-(--color-text-body)`}>{parsed.label}</span>
                </Fragment>
              );
            }
            return (
              <Fragment key={i}>
                <button
                  ...
                  className={`${timeCls} bg-transparent border-0 cursor-pointer transition-colors ${
                    isActive
                ...
                <button
                  onClick={() => handleSeek(parsed.seconds)}
                  className={`${labelCls} border-0 cursor-pointer transition-colors ${
                    isActive
  ...
  const contextEl = ctx ? (
    <section className="bg-(--color-surface-raised) rounded-xl p-5">
      <h2 className="text-xs font-bold uppercase tracking-wide text-neon-600 mb-2">{ctx.title}</h2>
      {ctx.groups?.length ? (
  ...
  const layoutToggle = (
    <div className="inline-flex items-center gap-0.5 p-0.5 rounded-lg border-2 border-(--color-border-soft) bg-(--color-surface-raised)">
      {(["plain", "design"] as const).map((mode) => (
        <button
          key={mode}
          onClick={() => setLayout(mode)}
          className={`text-xs font-bold px-3 py-1 rounded-md cursor-pointer transition-colors ${
            layout === mode
              ? "bg-neon-500 text-white"
              : "bg-transparent text-(--color-text-secondary) hover:text-neon-600"
          }`}
        >
          {mode === "plain" ? "Standard" : "Design"}
        </button>
      ))}
    </div>
  );
  ...
  const designSections: DesignSection[] = (
    [
      { id: "player", label: "Video", node: playerEl, sticky: true },
      { id: "meta", label: "Details", node: metaEl },
      jumpToEl ? { id: "jumpTo", label: "Jump To", node: jumpToEl } : null,
      tldrEl ? { id: "tldr", label: "TL;DR", node: tldrEl } : null,
      highlightsEl ? { id: "highlights", label: "Highlights", node: highlightsEl } : null,
      contextEl ? { id: "context", label: ctx?.title ?? "Context", node: contextEl } : null,
      { id: "notes", label: "My Notes", node: notesEl },
    ] as (DesignSection | null)[]
  ).filter((s): s is DesignSection => s !== null);

  return layout === "design" ? (
    // Design: full-height 3-column layout — the nav lives in the center column
    <Suspense
      fallback={
        <div className="flex justify-center py-16">
          <div className="w-8 h-8 border-3 border-neon-500 border-t-transparent rounded-full animate-spin" />
    ...
    >
      <DesignPlayground sections={designSections} nav={<Nav right={layoutToggle} />} />
    </Suspense>
  ) : (
    <>
      {/* Nav centered exactly like the dashboard */}
      <div className="max-w-3xl mx-auto px-6">
        <Nav right={layoutToggle} />
      </div>

      <div className="px-4 pb-12">
        {/* Standard: left pane (TL;DR + timestamps) + main pane (sticky video + content) */}
        <div className="flex flex-col lg:flex-row gap-6 lg:h-[calc(100vh-11rem)]">
          {/* left pane: TL;DR + timestamps, scrolls on its own */}
          <aside className="lg:w-1/3 lg:shrink-0 lg:overflow-y-auto lg:pr-1 space-y-5">
            {tldrEl}
            {jumpToEl}
          </aside>
          {/* main pane: scrolls on its own, video pinned to its top */}
          <div className="flex-1 min-w-0 lg:overflow-y-auto lg:pr-1">
            <div className="sticky top-0 z-10 bg-(--color-surface) pb-3">{playerEl}</div>
