...
function SummaryCard({
  ...
  async function handlePinClick(e: React.MouseEvent) {
    // The whole card is clickable — don't open the summary when pinning.
    e.stopPropagation();
    try {
      await createPin({ kind: "clip", layout: "tile", clipId: s.id, label: s.videoTitle });
      setPinned(true);
    } catch {
      // Already pinned (unique index) or offline — leave the control as-is.
    }
  }
  ...
  return (
    ...
    >
      ...
      {/* Pin to new tab — visible on hover, left of delete */}
      <button
        onClick={(e) => void handlePinClick(e)}
        className={`absolute right-9 bottom-2 text-[10px] font-bold px-1.5 py-0.5 rounded border bg-transparent cursor-pointer transition-all ${autoTagResult ? "hidden" : ""} ${
          pinned
            ? "border-neon-500 text-(--color-text) opacity-100"
            : "border-transparent text-(--color-text-faint) opacity-0 group-hover:opacity-100 hover:text-(--color-text)"
        }`}
        title={pinned ? "Pinned to new tab" : "Pin to new tab"}
      >
        {pinned ? (
          "Pinned"
        ) : (
